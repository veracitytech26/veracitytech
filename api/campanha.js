export default async function handler(req, res) {
  ...
  try {
    ...
    const { action, campanha_id, contato_id, dados } = req.body;
    ...
    // ── LISTAR CAMPANHAS ──
    if (action === 'listar') {
      const campRes = await fetch(`${SUPABASE_URL}/rest/v1/campanhas?user_id=eq.${userId}&order=created_at.desc&limit=10`, {
        headers: { 'apikey': SUPABASE_SERVICE, 'Authorization': `Bearer ${SUPABASE_SERVICE}` }
      });
