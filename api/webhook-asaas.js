export default async function handler(req, res) {
  ...
  try {
    ...
    // Busca cliente pelo externalReference (que será o user_id do Supabase)
    const externalRef = pagamento.externalReference;
    if (!externalRef) {
      console.log('Sem externalReference — ignorando');
    ...
    if (evento === 'PAYMENT_CONFIRMED' || evento === 'PAYMENT_RECEIVED') {
      ...
      // PACOTE 200 CONSULTAS
        ...
        update = { credits_total: atual + 200 };
      ...
      // PACOTE 500 CONSULTAS
        ...
        const atual = perfil.credits_total || 0;
        update = { credits_total: atual + 500 };
      ...
      // PACOTE 1000 CONSULTAS
        ...
        const atual = perfil.credits_total || 0;
        update = { credits_total: atual + 1000 };
      ...
      // VERACITY DISPARO
      else if (descricao.includes('disparo') || (valor >= 345 && valor <= 355)) {
        // Ativa por 30 dias
        const venc = new Date();
        venc.setDate(venc.getDate() + 30);
        update = { addon_disparo: true, disparo_vencimento: venc.toISOString() };
        emailAssunto = 'Veracity Disparo ativado — Veracity Intelligence';
        emailCorpo = `Seu Veracity Disparo foi ativado!\n\nNossa equipe entrará em contato em até 24h para configurar seu WhatsApp.\n\nWhatsApp suporte: (21) 97385-5107`;
      }
