export default async function handler(req, res) {
  ...
  try {
    ...
    // ANTI-DUPLICATA via Supabase — funciona em múltiplos servidores
    if (messageId) {
      try {
        var dupRes = await fetch(SUPABASE_URL + '/rest/v1/sdr_processados', {
          method: 'POST',
        ...
        // Se retornou 409 ou body vazio = duplicata, ignora
        if (dupRes.status === 409 || dupRes.status === 200 && dupRes.headers.get('content-length') === '0') {
          return res.status(200).json({ ok: true, motivo: 'duplicata' });
        }
        // Verifica se já existe
        var checkRes = await fetch(SUPABASE_URL + '/rest/v1/sdr_processados?message_id=eq.' + messageId + '&select=message_id', {
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + SUPABASE_KEY }
        });
        var checkData = checkRes.ok ? (await checkRes.json()) : [];
        if (checkData.length > 1) {
          return res.status(200).json({ ok: true, motivo: 'duplicata' });
        }
        // Limpa registros antigos (> 10 min)
        var limite10 = new Date(Date.now() - 10 * 60 * 1000).toISOString();
        fetch(SUPABASE_URL + '/rest/v1/sdr_processados?created_at=lt.' + limite10, {
          method: 'DELETE',
          headers: { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer ' + SUPABASE_KEY }
        });
      } catch(e) { console.log('Erro anti-duplicata:', e.message); }
    }
