  ...
  try {
    ...
    if (action === 'criar') {
      ...
      if (!contatosFiltrados.length) {
        return res.status(200).json({ ok: false, error: 'Todos os contatos já receberam mensagem.', duplicados });
      }
      ...
      // Salva empresa na tabela sdr_leads para o SDR IA usar o nome
      const leadsPayload = contatosFiltrados.map(c => {
        let tel = (c.telefone || '').replace(/[^0-9]/g, '');
        if (tel.startsWith('55') && tel.length > 11) tel = tel.slice(2);
        return { phone: '55' + tel, nome: c.nome, empresa: c.nome, status: 'novo', origem: 'campanha' };
      }).filter(l => l.phone.length >= 12);

      if (leadsPayload.length) {
        await fetch(`${SUPABASE_URL}/rest/v1/sdr_leads`, {
          method: 'POST',
          headers: { 'apikey': SUPABASE_SERVICE, 'Authorization': `Bearer ${SUPABASE_SERVICE}`, 'Content-Type': 'application/json', 'Prefer': 'resolution=ignore-duplicates' },
          body: JSON.stringify(leadsPayload)
        });
      }

      return res.status(200).json({ ok: true, campanha_id: camp.id, total: contatosFiltrados.length, duplicados });
    }
