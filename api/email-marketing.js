export default async function handler(req, res) {
  ...
  try {
    ...
    // Link do WhatsApp dinâmico — puxado do perfil do corretor logado
    const waLink = whatsappCorretor
      ? 'https://wa.me/55' + whatsappCorretor + '?text=Oi%2C+vi+sua+oferta+e+quero+saber+mais'
      : '';
    ...
    function montarHtml(nomeEmpresa, mensagem) {
      ...
      return '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">'
        ...
        + '}'
        + 'a.btn-copa { display:inline-block;background:' + verdeBrasil + ';color:#ffffff;font-size:15px;font-weight:700;padding:16px 48px;border-radius:6px;text-decoration:none;border:3px solid ' + amarelo + ';animation:pulso 1.5s infinite; }'
        ...
        + '<div style="font-size:11px;color:#ffffff;margin-top:2px;letter-spacing:1px;">CONDICAO ESPECIAL PLANO DE SAUDE EMPRESARIAL</div>'
        + '</td></tr>'
        ...
        // Botão animado com link dinâmico do corretor
        + (waLink
          ? '<tr><td style="padding:0 32px 32px;text-align:center;">'
          + '<a href="' + waLink + '" class="btn-copa" target="_blank">Falar no WhatsApp</a>'
          + '</td></tr>'
          : '')
