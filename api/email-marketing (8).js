export default async function handler(req, res) {
  ...
  try {
    ...
    function montarHtml(nomeCorretor, creci, susep, whatsappCorretor, emailCorretor, fotoUrl, corPrincipal, nomeEmpresa, mensagem) {
      const waNum = (whatsappCorretor || '').replace(/[^0-9]/g, '');
      const waLink = waNum ? 'https://wa.me/55' + waNum : '';
      const cor = corPrincipal || '#1240AB';

      const avatar = fotoUrl
        ? '<img src="' + fotoUrl + '" width="56" height="56" style="border-radius:50%;border:2px solid ' + cor + ';object-fit:cover;" alt="' + nomeCorretor + '">'
        : '<div style="width:56px;height:56px;border-radius:50%;background:' + cor + ';text-align:center;line-height:56px;font-size:22px;font-weight:700;color:#fff;">' + (nomeCorretor||'C').charAt(0).toUpperCase() + '</div>';
      ...
      return '<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>'
        ...
        + '<tr><td align="center">'
        + '<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border-radius:6px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">'

        // Cabeçalho — só dados do corretor
        + '<tr><td style="padding:28px 32px;border-bottom:3px solid ' + cor + ';">'
        ...
        + '<p style="font-size:15px;color:#1a1a2e;margin:0 0 20px 0;">Ola, <strong>' + nomeEmpresa + '</strong>!</p>'
        + '<div style="font-size:14px;color:#444;line-height:1.9;">' + mensagem.replace(/\n/g, '<br>') + '</div>'
        + '</td></tr>'

        // Botão WhatsApp
        + (waLink ? '<tr><td style="padding:0 32px 32px;text-align:center;"><a href="' + waLink + '" style="display:inline-block;background:#25D366;color:#ffffff;font-size:14px;font-weight:700;padding:14px 40px;border-radius:4px;text-decoration:none;">Falar no WhatsApp</a></td></tr>' : '')
        ...
        + '<div style="font-size:12px;color:#555;margin-top:4px;">Corretor de Saude Empresarial</div>'
    ...
    const corPrincipal    = perfil.cor_principal   || '#1240AB';
    ...
    if (action === 'enviar_um') {
      ...
      const htmlEmail   = montarHtml(nomeCorretor, creci, susep, whatsappCorretor, emailCorretor, fotoUrl, corPrincipal, nomeEmpresa, mensagemP);
    ...
    if (action === 'enviar_avulso') {
      ...
      const htmlEmail = montarHtml(nomeCorretor, creci, susep, whatsappCorretor, emailCorretor, fotoUrl, corPrincipal, 'Cliente', mensagem);
      const emailRes = await fetch('https://api.resend.com/emails', {
        method: 'POST',
        headers: { 'Authorization': 'Bearer ' + RESEND_KEY, 'Content-Type': 'application/json' },
        body: JSON.stringify({
