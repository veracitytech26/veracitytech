...
export default async function handler(req, res) {
  ...
  try {
    // Twilio pode enviar como form-urlencoded (webhook) ou JSON
    let body = {};
    if (req.method === 'POST') {
      if (typeof req.body === 'string') {
        try { body = JSON.parse(req.body); } catch(e) {
          // form-urlencoded do Twilio webhook
    ...
    // ── TWIML PARA O SDK ───────────────────────────────────────────────────
    // Twilio chama essa URL via TwiML App quando o navegador precisa receber
    if (body.CallSid && !action) {
...
<Response>
  <Dial>
    ...
    if (action === 'ligar') {
      ...
      // TwiML inline — quando cliente atender conecta com navegador do SDR
...
<Response>
  ...
  <Dial callerId="${TWILIO_FROM_NUMBER}" timeout="30">
    ...
    // ── STATUS CALLBACK ────────────────────────────────────────────────────
    if (action === 'status') {
      console.log('Call status:', body.CallSid, body.CallStatus, body.Duration);
