export default async function handler(req, res) {
  ...
  try {
    ...
    if (payload.somente_celular) {
      cnpjs = cnpjs.filter(function(e) {
        ...
        return tels.some(function(t) {
          // Formato: {tipo: "celular", ddd: "22", numero: "981201646"}
          if (t.tipo && t.tipo.toLowerCase() === 'celular') return true;
          // Número com 9 dígitos começando com 9
          var num = (t.numero || t.telefone || '').replace(/[^0-9]/g, '');
          if (num.length === 9 && num[0] === '9') return true;
          // Formato completo DDD+número
          var completo = (t.completo || '').replace(/[^0-9]/g, '');
          if (completo.length === 11 && completo[2] === '9') return true;
          return false;
        });
      });
    }
    ...
    const resposta = Object.assign({}, casaData, { cnpjs: cnpjs });
