export default async function handler(req, res) {
  ...
  // Verifica autenticação — aceita SERVICE_KEY diretamente (admin panel)
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ error: 'Nao autorizado' });
  const token = authHeader.replace('Bearer ', '');

  // Se o token for o SERVICE_KEY, é o admin — libera direto
  if (token !== SERVICE_KEY) {
    // Tenta validar como token de usuário Supabase
  ...
  if (action === 'listar_templates') {
    try {
      ...
      const templates = contents
        .filter(t => t.approvals && t.approvals.whatsapp && t.approvals.whatsapp.status === 'approved')
        .map(t => ({
          sid: t.sid,
          nome: t.friendly_name,
          status: t.approvals.whatsapp.status,
          categoria: t.approvals.whatsapp.category,
          body: t.types && t.types['twilio/card'] ? t.types['twilio/card'].body :
                t.types && t.types['twilio/text'] ? t.types['twilio/text'].body : '',
        }));
